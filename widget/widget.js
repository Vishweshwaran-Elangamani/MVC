(function () {
  ...
  const VERSION = "bubble-widget v3.2";
  ...
  const DISABLE_KEY = "bubble_workspace_disabled_" + WORKSPACE;
   
  /* prevent widget loading after deletion */
   
  if(localStorage.getItem(DISABLE_KEY) === "1"){
   console.warn("[Bubble] workspace disabled previously");
   return;
  }
  ...
  async function workspaceExists(workspace,key){
   
   try{
   
    const r = await fetch(`${API}/api/widget/config?workspace=${workspace}&key=${key}`);
   
    if(r.status===404){
   
     console.warn("[Bubble] workspace deleted");
  ...
  const exists = await workspaceExists(WORKSPACE,KEY);
  if(!exists) return;
  ...
  function removeWidget(){
   ...
   console.warn("[Bubble] workspace deleted → widget disabled permanently");
  ...
  const listNotes=()=>httpJson(`${API}/api/notes?${qs({ workspace:WORKSPACE,key:KEY,userId:USER_ID })}`);
  ...
  const addNote=(content)=>httpJson(`${API}/api/notes`,{
   ...
   body:JSON.stringify({ workspace:WORKSPACE,key:KEY,userId:USER_ID,content })
   
  });
   
  const delNote=(id)=>fetch(`${API}/api/notes/${id}?${qs({ workspace:WORKSPACE,key:KEY,userId:USER_ID })}`,{
  ...
  async function initAppearance(){
   ...
   try{
    ...
    const cfg=await httpJson(`${API}/api/widget/config?${qs({ workspace:WORKSPACE,key:KEY })}`);
  ...
  async function longPoll(){
   ...
   while(!stopLongPoll){
    ...
    try{
     ...
     const res=await fetch(`${API}/api/widget/config/long?${qs({ workspace:WORKSPACE,key:KEY,since:version })}`);
